...
def main(q):
    ...
    env = env_fl()
    agent = Agent(env.observation_space, env.action_space, BATCH_SIZE, LEARNING_RATE, TAU, GAMMA, DEVICE, False, DUEL)
    _, rewards_log = train(env, agent, RAM_NUM_EPISODE, EPS_INIT, EPS_DECAY, EPS_MIN, MAX_T)
    # np.save('{}_rewards{}.npy'.format(RAM_ENV_NAME,i), rewards_log)
...
if __name__ == '__main__':
    ...
    np.save('{}_results.npy'.format(RAM_ENV_NAME), Rew_log)

    rewards1000 = np.load('Optimizing_FL_with_RL_results.npy')
    ...
    # plt.legend(loc=4)
    plt.grid()
    plt.savefig("DDQN1.png", dpi=300, bbox_inches="tight")
    plt.show()
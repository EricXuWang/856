...
#         optimizer = torch.optim.SGD(net.parameters(), lr=Lr, momentum=self.args.momentum)
        ...
        for iter in range(E):
            batch_loss = []
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                # Put images and labels into cuda
                ...
                net.zero_grad()
                loss.backward()
#                 nn.utils.clip_grad_norm_(net.parameters(), max_norm=2)
                optimizer.step()
                
#                 # Print train information at every 10 batches
...
#         return net.state_dict(), net.parameters(), sum(epoch_loss) / len(epoch_loss)
